...
- **siem_config** (Block List, Max: 1) (see [below for nested schema](#nestedblock--siem_config))
...
<a id="nestedblock--siem_config"></a>
...
- **proofpoint_casb_config** (Block List) Configuration for log streaming to Proofpoint CASB for shadow IT processing.. (see [below for nested schema](#nestedblock--siem_config--proofpoint_casb_config))
- **qradar_http_config** (Block List) Configuration for log streaming to IBM QRadar platform. (see [below for nested schema](#nestedblock--siem_config--qradar_http_config))
- **s3_config** (Block List) Configuration for log streaming to an Amazon S3 bucket. (see [below for nested schema](#nestedblock--siem_config--s3_config))
- **splunk_http_config** (Block List) Configuration for log streaming to Self-Hosted / cloud Splunk. see [here](https://help.metanetworks.com/knowledgebase/log_streaming_for_splunk_self_hosted/#configuring-splunk-http-event-collector) for instructions on how to enable HTTP Event Collector on Self-Hosted Instance, and [here](https://help.metanetworks.com/knowledgebase/log_streaming_for_splunk_cloud/#configuring-splunk-http-event-collector) for instructions on how to enable HTTP Event Collector on Cloud Instance (see [below for nested schema](#nestedblock--siem_config--splunk_http_config))
- **syslog_config** (Block List) Configuration for log streaming in Syslog Common Event Format (CEF). (see [below for nested schema](#nestedblock--siem_config--syslog_config))

<a id="nestedblock--siem_config--proofpoint_casb_config"></a>
### Nested Schema for `siem_config.proofpoint_casb_config`

Required:

- **region** (String) Tenant region in Proofpoint CASB system. ENUM: `EU`, `US`.
- **tenant_id** (String) Your organization PCASB tenant ID in the tenant_ID format. Valid tenant ID format is *tenant<32 hex characters>*


<a id="nestedblock--siem_config--qradar_http_config"></a>
...
<a id="nestedblock--siem_config--s3_config"></a>
### Nested Schema for `siem_config.s3_config`

Required:

- **bucket** (String) Name of your AWS S3 bucket. Must have proper writing permissions and access rights for the Proofpoint logging.
- **compress** (Boolean) Whether to compress log objects using gzip
...
<a id="nestedblock--siem_config--splunk_http_config"></a>
...
<a id="nestedblock--siem_config--syslog_config"></a>
